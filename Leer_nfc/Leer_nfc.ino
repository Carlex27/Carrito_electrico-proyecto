...
byte bloque = 4; // Bloque donde se escribió la clave encriptada
...
void setup() {
    ...
    Serial.println("Acerque una tarjeta para validar...");
}
...
void loop() {
    ...
    // Convertir los datos encriptados a string
    String claveEncriptada = "";
    for (byte i = 0; i < 16; i++) {
        claveEncriptada += (char)buffer[i];
    }

    Serial.print("Clave encriptada leída: ");
    Serial.println(claveEncriptada);

    // Enviar la clave a Raspberry Pi para validación
    Serial.println(claveEncriptada);

    // Esperar respuesta de la Raspberry Pi
    String respuesta = "";
    ...
    if (respuesta == "OK") {
        ...
        digitalWrite(RELAY_PIN, motorEncendido ? LOW : HIGH);
